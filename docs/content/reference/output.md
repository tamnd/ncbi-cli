...
describes what users get. Pick a format with `-o`, or let ncbi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ncbi <command> -o table   # aligned columns for reading
ncbi <command> -o jsonl   # one JSON object per line, for piping
ncbi <command> -o json    # a single JSON array
ncbi <command> -o csv     # spreadsheet friendly
ncbi <command> -o tsv     # tab-separated
ncbi <command> -o url     # just the URL column
ncbi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ncbi <command> --fields id,title,url
```
...
```bash
ncbi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ncbi <command>            # a table, because this is a terminal
ncbi <command> | wc -l    # JSONL, because this is a pipe
```
